...
import { StrategyFactory } from '@sudoku/strategies';
import { activeStrategies } from '@sudoku/stores/activeStrategies'; 
import { STRATEGIES } from '@sudoku/constants';

// Add a store to track the last successful strategy
export const lastStrategyUsed = writable('');
...
function createUserGrid() {
	...
	grid.subscribe($grid => {
		...
		// Reset history stacks when a new game starts (base grid changes)
		undoStack.set([]);
		redoStack.set([]);
		recallStack_toundo.set([]);
		...
		lastStrategyUsed.set('');
        
        // Reset hints and candidates
        hints.reset();
        candidates.clear();
	});
	...
	return {
		...
		applyStrategy(strategyName = 'rectangle-elimination') {
			userGrid.update($userGrid => {
...
function getHintCells(grid) {
	const uniqueCells = [];
    const activeStrats = get(activeStrategies);
    // console.log("Debug: Active Strategies used in getHintCells:", activeStrats); // Debug log
    ...
    // Apply activated strategies sequentially, stop at first success
    for (const key of strategyKeys) {
        if (activeStrats[key]) {
            // console.log(`Debug: Trying strategy ${key}`); // Debug log
            try {
                const strategy = factory.createStrategy(key);
                // Use a temporary grid to test this strategy alone on the current state
                let tempGrid = JSON.parse(JSON.stringify(grid));
                tempGrid = strategy.solve(tempGrid);
                ...
                for (let x = 0; x < 9; x++) {
                    for (let y = 0; y < 9; y++) {
                        if (grid[x][y] === 0 && tempGrid[x][y] !== 0) {
                            uniqueCells.push({ x, y, value: tempGrid[x][y] });
                            hasChanges = true;
                        }
                    }
                }
                ...
                if (hasChanges) {
                    lastStrategyUsed.set(STRATEGIES[key]);
                    return uniqueCells; // Stop and return immediately
                }
    ...
    // Fallback: If no strategy yielded results (or none selected), 
    // maybe check for "naked singles" via basic logic if user wants hints?
    // Or just return empty if strategies failed to find anything.
    // The previous logic also did: if possibleValues.length <= hints.gethints() (e.g. 1)
    ...
    if (uniqueCells.length === 0) {
        ...
        for (let x = 0; x < 9; x++) {
            for (let y = 0; y < 9; y++) {
                if (grid[x][y] === 0) {
                    ...
                    if (possibleValues.length <= hints.gethints() && possibleValues.length > 0) {
        ...
        if (uniqueCells.length > 0) {
            lastStrategyUsed.set('Basic Logic');
        }
    }
